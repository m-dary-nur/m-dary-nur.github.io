...
const Button: React.FC<IButton> = ({ children, onClick, color = "gray", isLoading, disabled }) => (
   <button
      onClick={(): false | void => !isLoading && onClick()}
      disabled={disabled}
      className={`${disabled ? "opacity-50 cursor-not-allowed" : `hover:bg-${color}-600`} bg-${color}-500 ${
